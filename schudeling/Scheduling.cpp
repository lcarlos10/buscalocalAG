...
#include "ListaDuplaEncadeada.h" //Lista duplamente encadeada
...
int SetProblema(int intProblema);
//int LocalSearch4 implementado
int localSearch4(GAGenome& g, int originalScore);
void cpBasedSeedGA();
...
int main(int argc, char **argv)
{
    ...
    int P[MACHINE*JOB];
    //Verifica o makespan realizada na busca local.
...
int SetProblema(int intProblema){
    ...
    string strInstancia = "2";
        ...
        if (intProblema == 6){
            ...
            R[r++]=1; R[r++]=4; R[r++]=2; R[r++]=3; R[r++]=5;       T[t++]=63; T[t++]=28; T[t++]=46; T[t++]=26; T[t++]=52;
            R[r++]=4; R[r++]=3; R[r++]=5; R[r++]=2; R[r++]=1;       T[t++]=10; T[t++]=17; T[t++]=73; T[t++]=11; T[t++]=64;
            R[r++]=SetProblema3; R[r++]=2; R[r++]=4; R[r++]=5; R[r++]=1;       T[t++]=67; T[t++]=97; T[t++]=95; T[t++]=38; T[t++]=85;
            R[r++]=3; R[r++]=5; R[r++]=1; R[r++]=2; R[r++]=4;       T[t++]=95; T[t++]=46; T[t++]=59; T[t++]=65; T[t++]=93;
            R[r++]=3; R[r++]=5; R[r++]=4; R[r++]=2; R[r++]=1;       T[t++]=43; T[t++]=85; T[t++]=32; T[t++]=85; T[t++]=60;
            R[r++]=5; R[r++]=4; R[r++]=3; R[r++]=1; R[r++]=2;       T[t++]=49; T[t++]=41; T[t++]=61; T[t++]=66; T[t++]=90;
            R[r++]=2; R[r++]=1; R[r++]=4; R[r++]=5; R[r++]=3;       T[t++]=17; T[t++]=23; T[t++]=70; T[t++]=99; T[t++]=49;
...
int localSearch4(GAGenome& g, int originalScore)
{
        ...
        vector<int> bestSeed;
        int score;
        vector<int> contOcurr(JOB,0);
        vector<int> contOcurrZero(JOB,0);
        vector<int> contador(MACHINE,0);
        vector<int> contadorZero(MACHINE,0);
        string bestVizinho;

        //
        int P[MACHINE*JOB];
        //
        // Copia genoma para estrutura de coordenadas
        vector<int> row;
        vector<int> col;
        vector<int> colUp;
        vector<int> colDown;
        ...
        vector<int> colLeft;
        //
        for(int i=0; i<genome.width(); i++)//coluna
        {
            for(int j=0; j<genome.height(); j++)//linha
            {
                if(genome.gene(i,j) == 1)
                {
                    row.push_back(i);
        ...
        bool atualiza = false;
        ///////////////////////////////////////////////////////////////////////////////
        // Faz busca local nos quatro vizinhos do genoma (sup., inf., esq., dir.)
        ///////////////////////////////////////////////////////////////////////////////
        //
        while(melhora){
        //
        ...
        {
            // indices do nao nulo == 1
            int i = row[k];
            int j = col[k];
            // indices vizinho superior
            int idx_i = ((i+1) % JOB);
            int idx_j = ((j+MACHINE-1) % MACHINE); //PEGA VALOR NA LINHA DE CIMA
            //
            int val = SS[(idx_j*JOB)+i];
            SS[(idx_j*JOB)+i] = SS[(idx_j*JOB)+idx_i];
            SS[(idx_j*JOB)+idx_i] = val;
        }
        //
//        /////////////////////////////////////////////////////////////////////////////
...
//        }
        //
        ///////////////////////////////////////////////
        /// INTERPRETACAO DA SEMENTE POR OPERACAO   ///
        ///////////////////////////////////////////////
        for(int i = 0; i < JOB*MACHINE;i++){
            int job = SS[i]-1; // valor 0-index
            int maq = R[job*MACHINE+contOcurr[job]]-1; // 0-index
            contOcurr[job]++;
            //
            P[maq*JOB + job] = ++contador[maq];} // 1-index
        ///////////////////////////////////////////////////////
        contOcurr = contOcurrZero;
        contador = contadorZero;
        //
        // Calcula score
        score = factivel(P,R,1,T);
        //
        if(score < bestScore){
            bestScore = score;
            cout << "melhora viz. de cima" << endl;
            bestVizinho = "Vizinho de cima";
            //Temporario
            //for(int k=0; k<row.size(); k++)
             //       colUp.push_back(k) = ((col[k]+MACHINE-1) % MACHINE);
        ...
        for(int k=0; k<row.size(); k++)
        {
            ...
            int i = row[k];
            int j = col[k];
            // indices vizinho DE BAIXO
            int idx_i = ((i+1) % JOB);
            int idx_j = ((j+1) % MACHINE); //PEGA VALOR NA LINHA DE BAIXO
            //
            int val = SS[(idx_j*JOB)+i];
            SS[(idx_j*JOB)+i] = SS[(idx_j*JOB)+idx_i];
            SS[(idx_j*JOB)+idx_i] = val;
...
//        }
        ...
        if(score < bestScore){
            bestScore = score;
            cout << "melhora viz. de baixo" << endl;
            bestVizinho = "Vizinho de baixo";
            bestIndex = 1;
            //break;
        }
        //
        ...
        for(int k=0; k<row.size(); k++)
        {
            ...
            int i = row[k];
            int j = col[k];
            // indices vizinho da esquerda
            int idx_i = ((i+JOB-1) % JOB); //COLUNA DA ESQUERDA
            int idx_j = j;
            //
            int val = SS[(idx_j*JOB)+i];
            SS[(idx_j*JOB)+i] = SS[(idx_j*JOB)+idx_i];
...
//        }
        ...
        if(score < bestScore){
            bestScore = score;
            cout << "melhora viz. da esquerda" << endl;
            bestVizinho = "Vizinho da esquerda";
            bestIndex = 2;
            //break;
        }
        //
        ...
        for(int k=0; k<row.size(); k++)
        {
            ...
            int i = row[k];
            int j = col[k];
            // indices vizinho DA DIREITA
            int idx_i1 = ((i+1) % JOB);
            int idx_i2 = ((i+2) % JOB);
            int idx_j = j;
            //
            int val = SS[(idx_j*JOB)+idx_i1];
            SS[(idx_j*JOB)+idx_i1] = SS[(idx_j*JOB)+idx_i2];
            SS[(idx_j*JOB)+idx_i2] = val;
        }
        //
//        /////////////////////////////////////////////////////////////////////////////
//        ///   I N T E R P R E T A C A O   O R I G I N A L   D A   S E M E N T E   ///
...
//        }
        ...
        if(score < bestScore){
            bestScore = score;
            cout << "melhora viz. da direita" << endl;
            bestVizinho = "Vizinho da direita";
            bestIndex = 3;
            //break;
        }

        ///////////
        // ATULIZA ESTRUTURA DE COORDENADAS
        ////////////////////////////////////////////////////////////////
        if(bestIndex != -1){
            cout << "\n Melhorou solucao em Local Search: " << bestScore << " " << bestVizinho << endl;
            //getchar();
            if(bestIndex == 0){ //VIZINHO DE CIMA
                for(int k=0; k<row.size(); k++)
                    col[k] = ((col[k]+MACHINE-1) % MACHINE);
            }
            else if(bestIndex == 1){ //VIZINHO DE BAIXO
                for(int k=0; k<row.size(); k++)
                    col[k] = ((col[k]+1) % MACHINE);
            }
            else if(bestIndex == 2){ //VIZINHO DA ESQUERDA
                for(int k=0; k<row.size(); k++)
                    row[k] = ((row[k]+JOB-1) % JOB);
            }
            else if(bestIndex == 3){ //VIZINHO DA DIREIRA
                for(int k=0; k<row.size(); k++)
                    row[k] = ((row[k]+1) % JOB);
            }
        ...
        if(atualiza){
            ...
            // ZERA INDIVIDUO
            genome.unset(0,0,genome.width(),genome.height());
            // INSERE VALORES IGUAIS A 1
            for(int k=0; k<row.size(); k++)
            {
                genome.set(row[k],col[k],1,1);
            }
            //cout << "\n Atualiza: \n" << genome << endl;
            //getchar();
        }
